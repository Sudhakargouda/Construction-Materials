...
const getOrderedProductsBySeller = async (req, res) => {
    try {
        ...
        if (ordersWithSellerId.length > 0) {
            ...
            // Calculate totalNewPrice for each product
            orderedProducts.forEach(product => {
                if (product.price && product.price.cost) {
                    product.totalNewPrice = product.quantity * product.price.cost;
                } else {
                    product.totalNewPrice = null;
                }
            });
